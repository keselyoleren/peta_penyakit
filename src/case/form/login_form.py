...
class EmailAuthenticationForm(forms.Form):
    ...
    def __init__(self, request=None, *args, **kwargs):
        ...
        email_max_length = UserModel._meta.get_field(UserModel.EMAIL_FIELD).max_length or 254
        self.fields['email'].max_length = email_max_length
        self.fields['email'].widget.attrs['maxlength'] = email_max_length
        if self.fields['email'].label is None:
            self.fields['email'].label = capfirst(UserModel._meta.get_field(UserModel.EMAIL_FIELD).verbose_name)

    def clean(self):
        email = self.cleaned_data.get('email')
        password = self.cleaned_data.get('password')
        username = AccountUser.objects.filter(email=email).first()
    ...
    def get_invalid_login_error(self):
        return ValidationError(
            ...
            code='invalid_login',
            params={'email': UserModel._meta.get_field(UserModel.EMAIL_FIELD).verbose_name},
        )
